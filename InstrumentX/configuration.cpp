...
void Configuration::SetConfig(Config *p_config)
{
    ...
    ui->configuration_datarate->setText(QString::number(m_config->config_dataRate,10));
    ui->configuration_framerate->setText(QString::number(m_config->config_frameRate,10));
}
void Configuration::closeEvent(QCloseEvent *event)
{
    ...
    m_config->config_frameRate=ui->configuration_framerate->text().toInt(&ok,10);
...
void Configuration::paintEvent(QPaintEvent *event)
{
    ...
    painter.fillRect(QRect(0,10,this->width(),this->height()),QBrush(QColor(240,240,240)));     //设置背景颜色
}
